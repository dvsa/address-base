...
result=$(/usr/local/bin/aws s3 ls ${s3}| grep ${addressbaseversion} | grep csv.zip 2>&1) || log "Failed to list files from ${s3}: $result"

files=( $(/usr/local/bin/aws s3 ls ${s3} | grep ${addressbaseversion} | grep csv.zip | awk '{print $4}' 2>&1)) || log "Failed to extract file names"
